...
int main(int argc, char* const* argv){
	...
	float p1, p2 ;  
	bool test_mode = false, vect_4 = false, vect4_scan = false, ibrid_version = false; 

	while((opt = getopt(argc, argv, ":l:g:f:o:tvcin:s:d:p:r:")) != -1){
		switch(opt){
			case 'l':
				lws = atoi(optarg) ; 
				break ;
	...
	if(test_mode){ 
		...
		quickSortGpu(vec, nels, lws, nwg, &resource, test_mode, vect_4, vect4_scan, ibrid_version) ; 
	}
	else{

		vec = read_array_from_file( &nels, in_file) ;
		float*out_vec = quickSortGpu(vec, nels, lws, nwg, &resource, test_mode, vect_4, vect4_scan, ibrid_version) ;
